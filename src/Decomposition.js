...
function ExAnteDecomposition({ data, filters, visualization }) {
  ...
  useEffect(() => {
    ...
    d3.csv(
      `/wp-content/reactpress/apps/data-viz-geom/build/data/${type}/decomposition/decomposition_${country}_${year}_${type.replace(
        "-",
        ""
      )}.csv`
    )
  ...
  try {
    ...
    link.download = `${filters.country}_${filters.year}_decomposition_chart.png`;
    link.click();
  } catch (error) {
